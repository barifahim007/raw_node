...
const { maxCheckes } = require("../../helpers/evniroments");
    ...
    data.read("tokens", token, (err1, tokenData) => {
      if (!err1 && tokenData) {
        ...
        data.read("users", userPhone, (err2, userData) => {
          if (!err2 && userData) {
            handleToken._token.verify(token, userPhone, (tokenIsValid) => {
              if (tokenIsValid) {
                ...
                if (userChecks.length < maxCheckes) {
      ...
      if (!err && checkData) {
        const token =
          typeof requestProperties.headerObject.token === "string"
            ? requestProperties.headerObject.token
            : false;
        // handleToken._token.verify(token, phone, (tokenId) => {
        handleToken._token.verify(
          token,
